...
class PopHttpClient
{
    ...
    {
        $this->clientId = env('PDD_CLIENT_ID');
        $this->clientSecret = env('PDD_CLIENT_SCRIT');
    }
    ...
    public function syncInvoke($request, $access_token = "")
    {
        ...
        Log::info('pdd接口请求参数:' . json_encode($params));
    ...
    public function getRes($request)
    {
        $response = $this->syncInvoke($request);

        $content = $response->getContent();

        if (Arr::get($content, 'error_response') !== null) {
            Log::error('pdd接口异常返回:' . json_encode($content));
            throw new ServiceException($content['error_response']['error_msg']);
        }
